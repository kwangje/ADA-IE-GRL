...
class ADA_IE(sb.Brain):
    def compute_forward(self, batch, stage):
        batch = batch.to(self.device)
        wavs, fbanks, lens = self.prepare_features(batch.sig,stage)
        ...
        spk_output = self.modules.classifier_spk(enc_output)
        ag_output = self.modules.classifier_ag(enc_output)
        c_output = self.modules.discriminator_c(enc_output)

        return (spk_output, ag_output, c_output, enc_output)
    ...
    def prepare_features(self, wavs, stage):
        ...
        _, fbanks, _ = self.modules.mfcc(wavs)

        # if fbanks!=None: fbanks = self.modules.mean_var_norm(fbanks, lens)
        fbanks = self.modules.mean_var_norm(fbanks, lens)      

        return wavs, fbanks, lens
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        agegroup_encoded, _ = batch.agegroup_encoded
        c_encoded, _ = batch.c_encoded
        ...
        if stage == sb.Stage.TRAIN and hasattr(self.modules, "env_corrupt"):
            ...
            agegroup_encoded = torch.cat([agegroup_encoded, agegroup_encoded], dim = 0)
            c_encoded = torch.cat([c_encoded, c_encoded], dim=0)
            # triplet_encoded = torch.cat([triplet_encoded, triplet_encoded], dim=0)
            lens = torch.cat([lens, lens])

        # return (spk_output, ag_output, c_output, enc_output)
        spk_output, ag_output, c_output, enc_output = predictions

        loss = self.hparams.loss_metric(self.hparams.spk_loss_metric,
                                        spk_output,
                                        ag_output,
                                        c_output,
                                        speaker_encoded,
        ...
        if stage != sb.Stage.TRAIN:
            self.error_metrics.append(batch.id, ag_output, agegroup_encoded)
    ...
    def evaluate_batch(self, batch, stage):
        ...
        if stage != sb.Stage.TEST:
            ...
            spk_output, ag_output, c_output, enc_output = self.compute_forward(batch, stage=stage)
            ag_output = ag_output.squeeze(1)
            return enc_output, ag_output
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch in tqdm(
                ...
                Rewrite here
                """
                enc_output, ag_output  = self.evaluate_batch(batch, stage=Stage.TEST)
                enc_output = enc_output.unsqueeze(1)
                ag_scores = [ag_output[i].item()
                             for i in range(ag_output.shape[0])]
                for i, seg_id in enumerate(batch.id):
                    ag_dict[seg_id] = ag_scores[i]
...
class SPK_Decoder(torch.nn.Module):
...
class C_Discriminator(torch.nn.Module):
    def __init__(
            self,
            input_size,
            activation=torch.nn.LeakyReLU,
...
class AG_Decoder(sb.nnet.containers.Sequential):
    def __init__(
            self,
            input_shape,
            activation=torch.nn.LeakyReLU,
